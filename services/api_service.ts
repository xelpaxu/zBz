...
const API_BASE_URL = `http://${IP_ADDRESS_NICOLE}:8000`;
...
export interface AnalysisResponse {
    ...
    reasoning: string;   // <--- Added from Gemma 3
    accuracy: string;    // <--- Added from YOLO max confidence
    message: string;
    processed_image?: string; // <--- The image with bounding boxes drawn
}
 ...
 */
export const analyzeImage = async (imageUri: string, description: string): Promise<AnalysisResponse> => {
